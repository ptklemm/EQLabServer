 ...
 * @param {string} options.port Database port to connect to
...
function mysqldump(options, callback) {
    ...
    function streamSchemaDump(table) {
        connection.query('SHOW CREATE TABLE ??', table, function (err, results) {
            ...
            var schemaDump = results[0]['Create Table'];
            writeStream.write(schemaDump + ';\n');
        });
    }
    ...
    if (options.version) writeStream.write('-- ' + 'Version: ' + options.version + '\n--\n');
