...
/************************ ZONE EDITOR *********************************/
// zone_router.get("/:zoneName/graphics", (req, res, next) =>
// {
//     const filePath = __filesdir + `/graphics/${req.params.zoneName}/${req.params.zoneName}.zip`;
...
zone_router.get("/:zoneName/zonegraphics", (req, res, next) =>
{
    const filePath = __filesdir + `/graphics/${req.params.zoneName}/${req.params.zoneName}.json`;
    // add check for file exist
    if (filePath) 
    {
        res.status(200).type('json').sendFile(filePath);
    } 
    else 
    {
        next(error);
...
zone_router.get("/:zoneName/texture/:textureName", (req, res, next) =>
{
    const filePath = __filesdir + `/graphics/${req.params.zoneName}/${req.params.textureName}`;
    // add check for file exist
    if (filePath) {
        res.status(200).type('image/bmp').sendFile(filePath);
    } else {
        next(error);
    }
});
